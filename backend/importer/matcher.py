...
COUNTRY_NAME_ALIASES = {
    ...
    'UNITED ARAB EMIRATES': 'UAE', 'UNITED ARAB EMERATES': 'UAE',
    'EAU': 'UAE', 'EMIRATES': 'UAE',
    ...
    'BRUNEI DARUSSALAM': 'BRU', 'BRUNEI': 'BRU',
    'KYRGYZ REPUBLIC': 'KGZ', 'KYRGYZSTAN': 'KGZ',
    'MACAU CHINA': 'MAC', 'MACAU': 'MAC', 'MACAO': 'MAC', 'MACAO CHINA': 'MAC',
    # Formal / Olympic-style names
...
def find_matching_swimmer(parsed_result, threshold=92, category='', meet_date=None):
    ...
    if birth_year:
        ...
        exact_year = [s for s in candidates
                      if _get_swimmer_birth_year(s) == birth_year]
        if len(exact_year) > 1:
            # Same name AND same birth year (two "Mohamed Amine DRIDI"
            # b.2010) — the club is the only tiebreaker.
