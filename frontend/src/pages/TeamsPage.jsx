...
export default function TeamsPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${ids.length} team(s)?\n\nResults and swimmers keep their club names — only the team profiles (logos, info, trophies) are removed. This cannot be undone.`)) return
  ...
  const columns = [
    {
      ...
      label: (
        <input type="checkbox" checked={allSelected} onChange={toggleAll}
          onClick={e => e.stopPropagation()} className="w-4 h-4 accent-sky-600 cursor-pointer" />
      ),
      render: (row) => (
        ...
        <div className="flex flex-wrap gap-2">
          {selected.size > 0 && (
            <button onClick={handleBulkDelete} disabled={bulkDeleting}
              className="bg-red-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-red-700 disabled:opacity-40 flex items-center gap-1.5">
              {bulkDeleting ? (
                <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Deleting...</>
              ) : (
                <>
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L5.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
